		...
		.extend(
				...
				(function() {
					...
					function endsWith(pattern, position) {
						...
						position = Object.isNumber(position) ? position
								: this.length;
						if (position < 0)
							position = 0;
					...
					setRequestHeaders : function() {
						...
						for ( var name in headers)
							if (headers[name] != null)
								this.transport.setRequestHeader(name,
										headers[name]);
					},
...
(function(GLOBAL) {
	...
	GLOBAL.Element.Methods.Simulated.hasAttribute = PROBLEMATIC_HAS_ATTRIBUTE_WITH_CHECKBOXES ? hasAttribute_IE
			: hasAttribute;
...
(function() {
	...
	if (!Object.isUndefined(document.documentElement.currentStyle)
	...
	function clonePosition(element, source, options) {
		...
		if (options.setLeft)
			styles.left = (p[0] + pageXY.x - delta[0] + options.offsetLeft)
					+ 'px';
		if (options.setTop)
			styles.top = (p[1] + pageXY.y - delta[1] + options.offsetTop)
					+ 'px';
...
/*
 * ! Sizzle CSS Selector Engine v1.10.18 http://sizzlejs.com/
 * 
...
(function(window) {
	...
	runescape = new RegExp("\\\\([\\da-f]{1,6}" + whitespace + "?|("
		...
		return high !== high || escapedWhitespace ? escaped : high < 0 ? String
				.fromCharCode(high + 0x10000) : String.fromCharCode(
	...
	setDocument = Sizzle.setDocument = function(node) {
		...
		sortOrder = hasCompare ? function(a, b) {
			...
			if (compare
				...
				return sortInput ? (indexOf.call(sortInput, a) - indexOf.call(
						sortInput, b)) : 0;
			}
					...
					if (!aup || !bup) {
						return a === doc ? -1 : b === doc ? 1 : aup ? -1
								: bup ? 1 : sortInput ? (indexOf.call(
										sortInput, a) - indexOf.call(sortInput,
										b)) : 0;
	...
	Sizzle.matchesSelector = function(elem, expr) {
		...
		if (support.matchesSelector && documentIsHTML
			...
			try {
				...
				if (ret || support.disconnectedMatch || elem.document
	...
	Sizzle.attr = function(elem, name) {
		...
		var fn = Expr.attrHandle[name.toLowerCase()], val = fn
				&& hasOwn.call(Expr.attrHandle, name.toLowerCase()) ? fn(elem,
			...
			if ((matcher = Expr.relative[tokens[i].type])) {
				...
				if (matcher[expando]) {
					...
					return setMatcher(i > 1 && elementMatcher(matchers), i > 1
							&& toSelector(tokens.slice(0, i - 1).concat({
	...
	function matcherFromGroupMatchers(elementMatchers, setMatchers) {
		var bySet = setMatchers.length > 0, byElement = elementMatchers.length > 0, superMatcher = function(
			...
			var elem, j, matcher, matchedCount = 0, i = "0", unmatched = seed
					...
					|| byElement && Expr.find["TAG"]("*", outermost), dirrunsUnique = (dirruns += contextBackup == null ? 1
